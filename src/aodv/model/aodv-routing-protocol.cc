...
RoutingProtocol::RoutingProtocol ()
  : m_rreqRetries (2),
    ...
    maliciousNode (Ipv4Address("0.0.0.0")),
  ...
  if (result)
    {
      ...
      std::cout<<"Add packet " << p->GetUid () << " to queue. Protocol " << (uint16_t) header.GetProtocol ()<<"\n";
      RoutingTableEntry rt;
      ...
      if (!result || ((rt.GetFlag () != IN_SEARCH) && result))
        {
          ...
          if (IsDetectBlackhole && fakeAddress.IsEqual(Ipv4Address("0.0.0.0")))
            {
              std::cout<<"Sending fake RREQ to detect Blackhole from "<< header.GetSource()<< " \n";
              fakeAddress = Ipv4Address("10.1.1.99");
              SetFakeAddress(fakeAddress);
              SendRequest(fakeAddress);
            }
          else
          {
            std::cout<<"Send new RREQ for outbound packet to " << header.GetDestination ()<<"\n";
              ...
              if (lcb.IsNull () == false)
                {
                  ...
                  //std::cout<<"Broadcast local delivery to " << iface.GetLocal ()<<" from "<<origin<<"\n";
                  lcb (p, header, iif);
              ...
              if (header.GetTtl () > 1)
                {
                  ...
                  std::cout<<"Forward broadcast. TTL " << (uint16_t) header.GetTtl ()<<"\n";
      ...
      if (lcb.IsNull () == false)
        {
          ...
          std::cout<<"Unicast local delivery to " << dst<<" from "<<origin<<"\n";
          lcb (p, header, iif);
        }
      else
        {
  ...
  if (m_ipv4->IsForwarding (iif) == false)
    {
      ...
      std::cout<<"Forwarding disabled for this interface \n";
      ecb (p, header, Socket::ERROR_NOROUTETOHOST);
      return true;
    }
                             ...
                             UnicastForwardCallback ucb, ErrorCallback ecb)
{
  ...
  //serangan
  //drop paket ketika dilakukan forwarding
  if(IsMalicious)
    {
      if (m_routingTable.LookupRoute (dst, toDst))
      {
        Ptr<Ipv4Route> route = toDst.GetRoute ();
        std :: cout<<route->GetSource ()<<" forwarding to " << dst << " from " << origin << " packet " << p->GetUid ()<<" Packet dropped\n";
        return false;
      }
      ...
      if (toDst.GetFlag () == VALID)
        {
          ...
          //deteksi
          //cek blacklist
          if(route-> GetGateway()== GetMaliciousNode()){
            std::cout<<GetMaliciousNode()<<" blacklist node, drop route !\n";
            m_routingTable.DeleteRoute (dst);
            SendRerrWhenNoRouteToForward (dst, 0, origin);
            return false;
          }
          NS_LOG_LOGIC (route->GetSource () << " forwarding to " << dst << " from " << origin << " packet " << p->GetUid ());
          std::cout<<route->GetSource () << " forwarding to " << dst <<" from " << origin << " packet " << p->GetUid ()<<"\n";
          /*
  ...
  std::cout<<"route not found to " << dst << ". Send RERR message.\n";
  NS_LOG_DEBUG ("Drop packet " << p->GetUid () << " because no route to forward it.");
  SendRerrWhenNoRouteToForward (dst, 0, origin);
  return false;
}
...
RoutingProtocol::NotifyAddAddress (uint32_t i, Ipv4InterfaceAddress address)
{
  ...
  if (l3->GetNAddresses (i) == 1)
  ...
  else
    {
      ...
      std::cout<<"AODV does not work with more then one address per each interface. Ignore added address\n";
    }
}
...
RoutingProtocol::NotifyRemoveAddress (uint32_t i, Ipv4InterfaceAddress address)
{
  ...
  if (socket)
  ...
  else
    {
      ...
      std::cout<<"Remove address not participating in AODV operation\n";
    }
}
         ...
         m_socketAddresses.begin (); j != m_socketAddresses.end (); ++j)
    {
      ...
      if (iface.GetMask () == Ipv4Mask::GetOnes ())
      ...
      else
        {
          ...
          //std::cout<<GetMaliciousNode()<<" alamat yang diblok\n";
        }
...
RoutingProtocol::ScheduleRreqRetry (Ipv4Address dst)
{
  ...
  if (rt.GetHop () < m_netDiameter)
  ...
  else
    {
      ...
      std::cout<<"Applying binary exponential backoff factor \n";
      retry = m_netTraversalTime * (1 << backoffFactor);
    }
  m_addressReqTimer[dst].Schedule (retry);
  NS_LOG_LOGIC ("Scheduled RREQ retry in " << retry.GetSeconds () << " seconds");
  std::cout<<"Scheduled RREQ retry in " << retry.GetSeconds () << " seconds\n";
}
    ...
    case AODVTYPE_RREP:
      {
        ...
        //deteksi
        //cek blacklist malicious node
        if(sender != GetMaliciousNode())
        {
          RecvReply (packet, receiver, sender);
          break;
        }
        std::cout<<GetMaliciousNode()<<" blacklist node, drop rrep ! receiver "<<receiver<<"\n";
...
RoutingProtocol::RecvRequest (Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
  ...
  std::cout<<receiver<< " receive RREQ with hop count " << static_cast<uint32_t> (rreqHeader.GetHopCount ())<< " ID " << rreqHeader.GetId ()<< " to destination " << rreqHeader.GetDst () <<"\n";
  ...
  if (IsMalicious || m_routingTable.LookupRoute (dst, toDst))
    {
      ...
      //serangan
      //membuat routing table entry dengan nilai sequence lebih tinggi dari rreq dan hop count 1
          ...
          if (IsMalicious || (!rreqHeader.GetDestinationOnly () && toDst.GetFlag () == VALID))
             {
              ...
              if(IsMalicious)
              {
                ...
                std::cout<<"Send RREP from " << receiver<<" Destination "<<dst<<"\n";
  ...
  if (gratRep)
    {
      ...
      std::cout<<"Send gratuitous RREP " << packet->GetUid ()<<"\n";
  ...
  RoutingProtocol::RecvReply (Ptr<Packet> p, Ipv4Address receiver, Ipv4Address sender)
{
  ...
  //deteksi
  //pelaku dengan tujuan rrep alamat palsu
  if (IsDetectBlackhole && dst == Ipv4Address("10.1.1.99")&& GetMaliciousNode() == Ipv4Address("0.0.0.0"))
    {
      std::cout<<"Black hole detected !\n";
      Ipv4Address pelaku = sender;
      SetMaliciousNode(pelaku);
      std :: cout << GetMaliciousNode() << " Malicious Node ! receiver "<< receiver << " \n";   
      return;
    }
  ...
  RoutingTableEntry newEntry (/*device=*/ dev, /*dst=*/ dst, /*validSeqNo=*/ true, /*seqno=*/ rrepHeader.GetDstSeqno (),
                                          ...
                                          /*nextHop=*/ sender, /*lifeTime=*/ rrepHeader.GetLifeTime (), GetMaliciousNode());
  //RoutingTableEntry toDst;
  ...
  std::cout<<"receiver " << receiver << " origin " << rrepHeader.GetOrigin ()<<"\n";
  ...
  if (toDst.GetRreqCnt () == m_rreqRetries)
    {
      ...
      std::cout<<"route discovery to " << dst << " has been attempted RreqRetries (" << m_rreqRetries << ") times with ttl " << m_netDiameter<<"\n";
      m_addressReqTimer.erase (dst);
      m_routingTable.DeleteRoute (dst);
  ...
  if (toDst.GetFlag () == IN_SEARCH)
    {
      ...
      std::cout<<"Resend RREQ to " << dst << " previous ttl " << toDst.GetHop ()<<"\n";
      SendRequest (dst);
    }
  else
    {
      ...
      if (!rerrHeader.AddUnDestination (i->first, i->second))
        {
          ...
          std::cout<<"Send RERR message with maximum size.\n";
          TypeHeader typeHeader (AODVTYPE_RERR);
          Ptr<Packet> packet = Create<Packet> ();
          SocketIpTtlTag tag;
          tag.SetTtl (1);
  ...
  if (m_rerrCount == m_rerrRateLimit)
    {
      ...
      NS_LOG_LOGIC ("RerrRateLimit reached at " << Simulator::Now ().GetSeconds () << " with timer delay left "
                                                ...
                                                << "; suppressing RERR");
      std::cout<<"RerrRateLimit reached at " << Simulator::Now ().GetSeconds () << " with timer delay left "<< m_rerrRateLimitTimer.GetDelayLeft ().GetSeconds () << "; suppressing RERR\n";
      return;
    }
  RerrHeader rerrHeader;
  ...
  else
    {
      for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator i =
             m_socketAddresses.begin (); i != m_socketAddresses.end (); ++i)
        {
          ...
          std::cout<<"Broadcast RERR message from interface " << iface.GetLocal ()<<"\n";
          // Send to all-hosts broadcast if on /32 addr, subnet-directed otherwise
          Ipv4Address destination;
          if (iface.GetMask () == Ipv4Mask::GetOnes ())
            {
      ...
      if (m_routingTable.LookupValidRoute (precursors.front (), toPrecursor))
        {
          ...
          std::cout<<"one precursor => unicast RERR to " << toPrecursor.GetDestination () << " from " << toPrecursor.GetInterface ().GetLocal ()<<"\n";
