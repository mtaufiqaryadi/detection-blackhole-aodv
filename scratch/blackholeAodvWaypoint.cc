...
NS_LOG_COMPONENT_DEFINE ("BlackholeWaypoint");
...
int main(int argc, char const *argv[])
{
    ...
    AnimationInterface anim ("blackholeAodvWaypoint.xml");
    ...
    for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats.begin (); i != stats.end (); ++i)
        ...
        if ((t.sourceAddress=="10.1.1.2"))
        {
            ...
            std::cout << "  Packet Loss :   " << ((float)i->second.txBytes - (float)i->second.rxBytes) / (float)i->second.txBytes * 100 << "%\n";
    ...
    monitor->SerializeToXmlFile("simulationBlackWaypoint.flowmon", true, true);
